...
def compare_json(snapshot1_file, snapshot2_file, ignored_attributes):
    ...
    nodes_1 = {f"{node['nodeId']['nodeId']}": node for node in snapshot1_data['snapshot']}
    nodes_2 = {f"{node['nodeId']['nodeId']}": node for node in snapshot2_data['snapshot']}
    
    diff_items = []
    for node_id, node_1 in nodes_1.items():
        if node_id in nodes_2:
            node_2 = nodes_2[node_id]
            diff_attributes = compare_items(node_1, node_2, ignored_attributes)
            if diff_attributes:
                diff_items.append({"nodeId": node_id, "attributes": ", ".join(diff_attributes)})
        else:
            diff_items.append({"nodeId": node_id, "attributes": "NodeId missing in second snapshot"})
...
@log_entry_exit
def categories_compare(original_snapshot_path, modified_snapshot_path, ignored_attributes):
