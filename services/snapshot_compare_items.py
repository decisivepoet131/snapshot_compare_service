...
def compare_json(snapshot1_file, snapshot2_file, ignored_attributes):
    ...
    items1 = {f"{item['itemId']['itemCode']}": item for item in snapshot1_data['snapshot']}
    items2 = {f"{item['itemId']['itemCode']}": item for item in snapshot2_data['snapshot']}
...
@log_entry_exit
def items_compare(original_snapshot_path, modified_snapshot_path, ignored_attributes):
